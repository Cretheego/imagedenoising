...
randNum = 5;
sigma1 = 30;
sigma = sigma1/255; %0.01;
%maindir = uigetdir( '选择一个文件夹' );
cleandir = 'D:\\jupyter_notebook\\lena\\clean\\test';
noisydir = ['D:\\jupyter_notebook\\lena\\noisy\\gauss\\test\\noisy_test',num2str(sigma1+0.3)];
denoisydir = ['D:\\jupyter_notebook\\lena\\denoise\\denoise_cum',num2str(sigma1+0.1053)];
...
for i = 1 : randNum 
    ...
    TV_psnr = TV_psnr + psnr(fima_TV/255.0,ima);
    TV_ssim = TV_ssim + ssim(fima_TV/255.0,ima);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    eima = rgb2gray(double(imread(file_name3))/255.0);
...
subplot(2,2,4),imagesc(rima-eima),title('residuals');
...
subplot(2,2,4),imagesc(rima-fima_BM3D),title('residuals');
...
% subplot(2,2,4),imagesc(rima-eima1),title('residuals');
